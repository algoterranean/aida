...
namespace
{
	...
	const TCHAR* GAIDASystemPrompt = TEXT(
		...
		"pipes are one ~56 m run. Names match loosely ('coal miner' finds 'Miner Mk.3') and a machine "
		"whose port is taken resolves to its manifold's open end — pass the player's own words. When "
		"they are near both things, name both sides. Do NOT interrogate the player about positions, "
		"port types, or what carries what — call the tool and relay what it found or why it failed.\n"
...
void UAIDAOrchestrator::RegisterActionTools()
{
	...
	Tools.Register({
		TEXT("propose_build"),
		TEXT("PROPOSE placing buildables. Nothing is built until a player with act permission approves. Returns a dry-run report (count, cost, validity, the RESOLVED origin in metres) and a proposalId. TWO SPEC FORMS. v1 single grid: {version:1, buildable:'display name', origin?:{x,y,z metres}, yawDeg:0|90|180|270, grid:{countX,countY,gapX?,gapY?}, rowDirection?:'left'|'right'|'forward'|'back'|compass (the row/grid-X axis, resolved against the player's FACING — 'a row left to right' = rowDirection 'right'; overrides yawDeg), followTerrain?:bool} — ALWAYS the form for a row/line/grid of IDENTICAL buildables ('three refineries in a line' = v1 with grid countX 3), never a composite. v2 COMPOSITE — the form for MULTI-PART structures with DIFFERENT parts (buildings, reference-image reconstructions): {version:2, origin?:{x,y,z}, yawDeg:0, parts:[{buildable:'display name', at:{x,y,z metres RELATIVE to the origin — z stacks upward}, yawDeg?:0, grid?:{countX,countY,stepX?,stepY?}}, ...]} — up to 32 parts place together, preview together, and get ONE approval; part offsets rotate with the composite yawDeg. OMIT origin to build where the requesting player is aiming (falls back to their position) — never ask the player for coordinates. v1 grids ALWAYS pack edge to edge — the server computes the pitch from each buildable's true footprint, and any stepX/stepY you send is IGNORED. The ONLY spacing control is gapX/gapY: metres of clear space BETWEEN machines, and you pass it ONLY when the player explicitly asked ('leave 5 m between them'); 'in a row/line/next to each other/compactly' means NO gap fields at all (a 'Foundation (2 m)' tile is 8x8 m; the 2 m is THICKNESS — stack floors with at.z, e.g. next storey at z 4 on '4 m' walls). v1 grids are FLAT at the origin's height (followTerrain:true ONLY if the player asks to trace the ground). v1 machines that need power are wired AUTOMATICALLY (poles + lines + grid tie; power:false to skip; pole:'display name' to override) — v2 composites are NOT auto-wired: include poles as parts and wire later. Costs are paid from central storage (dimensional depot) FIRST and then the REQUESTING PLAYER'S INVENTORY — materials in the player's pockets count toward affordability; never tell a player to move items into storage first. BLOCKED GROUND NEVER FAILS A PROPOSAL: uneven terrain/obstructions produce a proposal anyway (invalidCount + firstFailures in the result) with the ghost preview up — tell the player to NUDGE the ghost onto clear ground before approving (approving as-is builds only the valid placements and refunds the rest); never report blocked ground as 'can't build there'. REVISING: when the player asks to CHANGE a pending proposal ('make it 20', 'use mk.2', 'actually 2 rows'), read its spec from get_proposal_status, merge the change, and call this tool again with replaceProposalId set to the old id — the ghost swaps to the revision, no reject needed. To ADD A MANIFOLD to a pending build use propose_manifold with forProposalId instead."),
		...
		[this](const TSharedRef<FJsonObject>& Args, const FAIDAToolContext& Ctx) -> FAIDAToolResult
		{
			...
			if (bAimDerivedOrigin)
			{
				...
				// Composites snap by their ANCHOR part (parts[0]) — the rest ride its offsets. v1
				// grids pack edge to edge (footprint + explicit gap; model-guessed steps ignored).
				const FAIDAGridSpec& AnchorGrid = bComposite ? Spec.Parts[0].Grid : Spec.Grid;
				const double StepXCm = bComposite
					? FMath::Max(AnchorGrid.StepXM, Recipe.FootprintXM) * 100.0
					: (Recipe.FootprintXM + AnchorGrid.GapXM) * 100.0;
				const double StepYCm = bComposite
					? FMath::Max(AnchorGrid.StepYM, Recipe.FootprintYM) * 100.0
					: (Recipe.FootprintYM + AnchorGrid.GapYM) * 100.0;
				UE_LOG(LogAIDA, Log, TEXT("[actions] grid step: %.1f x %.1f m (gap %.1f x %.1f, footprint %.1f x %.1f) for %s"),
					StepXCm / 100.0, StepYCm / 100.0, AnchorGrid.GapXM, AnchorGrid.GapYM,
			...
			if (Spec.bPower && Placements.Num() > 0)
			{
				...
				{
					const double EffStepXCm = (Recipe.FootprintXM + Spec.Grid.GapXM) * 100.0; // matches ExpandGrid's v1 pitch
					const double EffStepYCm = (Recipe.FootprintYM + Spec.Grid.GapYM) * 100.0;
	...
	Tools.Register({
		...
		TEXT("Run a belt (or pipe) between two things that are ALREADY BUILT: from a free OUTPUT port to a free INPUT port — a miner's output into a generator's fuel input, a splitter row's open end into a machine, a storage container into a constructor. THE tool for 'connect A to B / belt this to that' when both ends exist. Endpoints: from = the source (items leave it), to = the destination (items enter it); each is {buildable?: name, center?: {x,y,z metres}, radiusM?}. Names match LOOSELY (any word — 'coal miner' finds 'Miner Mk.3'), and a named machine whose port is already taken by its manifold resolves to that row's open end automatically — pass the player's own words, do not translate or interrogate. Give a buildable name (searched near the player) OR a center, and you may OMIT ONE endpoint entirely — the thing the PLAYER IS AIMING AT fills it. When the player is near BOTH ('I'm standing next to both'), just name both sides from their sentence: {from:{buildable:'miner'}, to:{buildable:'generator'}}. If the stated direction is impossible (the 'from' has no free output) but the reverse works, the server flips it and says so — items always flow output -> input. Belts chain runs automatically for long distances (up to ~2 km, charged as built); pipes are one run (~56 m). Spec: {version:1, kind?: 'belt' (default) | 'pipe', from?, to?, transport?: belt/pipe display name}. NOT for tapping a flowing belt mid-run (propose_belt_tap) or plumbing a whole machine row (propose_manifold). Undo removes the run."),
